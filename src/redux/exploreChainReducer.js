...
const ensureChainStateExists = (state, chainId) =>
  (state.chains[chainId] ? state : state.setIn(['chains', chainId], emptyChainState(chainId)));

const selectChain = (state, chainId) =>
  (chainId ? ensureChainStateExists(state, chainId).setIn(['selectedChainId'], chainId) : state);

const updateChain = (state, action) => {
  const {
    payload: { chainAlias: chainId, state: rawChainState },
  } = action;
  if (rawChainState && rawChainState.interbit && rawChainState.interbit.blocks) {
